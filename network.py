...
class HTDLinkNet34(nn.Module):
    def __init__(self, num_classes=1):
        super(HTDLinkNet34, self).__init__()
        ...
        vote_index_3 = hough_transform(rows=32, cols=32, theta_res=3, rho_res=1)
    ...
    def forward(self, x):
        ...
        d4 = self.decoder4(e4) + self.ht_3(e3)
        d3 = self.decoder3(d4) + self.ht_2(e2)
        d2 = self.decoder2(d3) + self.ht_1(e1)
        d1 = self.decoder1(d2)
...
class MHTDLinkNet34(nn.Module):
    def __init__(self, num_classes=1):
        super(MHTDLinkNet34, self).__init__()
        ...
        self.distance_branch = nn.Sequential(nn.ConvTranspose2d(filters[0], 32, (4, 4), (2, 2), (1, 1)),
                                             nn.ReLU(inplace=True),
                                             nn.Conv2d(32, 32, (3, 3), padding=1),
                                             nn.ReLU(inplace=True),
                                             nn.Conv2d(32, 1, (3, 3), padding=1))
        self.direction_branch = nn.Sequential(nn.ConvTranspose2d(filters[0], 32, (4, 4), (2, 2), (1, 1)),
                                              nn.ReLU(inplace=True),
                                              nn.Conv2d(32, 32, (3, 3), padding=1),
                                              nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        dis = self.distance_branch(d1)
        dir = self.direction_branch(d1)

        return torch.sigmoid(out), torch.sigmoid(dis), torch.sigmoid(dir)


class MDLinkNet34(nn.Module):
    def __init__(self, num_classes=1):
        super(MDLinkNet34, self).__init__()
